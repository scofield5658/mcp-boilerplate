...
try:
    __version__ = version("mcp-boilerplate")
...
@click.version_option(__version__, prog_name="mcp-boilerplate")
@click.command()
@click.option(
    "-v",
    "--verbose",
...
async def run_stdio_server():
    ...
    import os
    user_id = os.getenv("SOME_ENV_VAR")
    app = create_mcp_server(user_id=user_id)
...
async def run_sse_server(host: str, port: int):
    ...
    async def handle_sse(request: Request):
        user_id = request.headers.get("some_env_var")
        app = create_mcp_server(user_id=user_id)
